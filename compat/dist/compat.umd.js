!(function (n, e) {
	'object' == typeof exports && 'undefined' != typeof module
		? e(exports, require('preact'), require('preact/hooks'))
		: 'function' == typeof define && define.amd
			? define(['exports', 'preact', 'preact/hooks'], e)
			: e(((n || self).preactCompat = {}), n.preact, n.preactHooks);
})(this, function (n, e, t) {
	function r(n, e) {
		var r = e(),
			o = t.useState({ t: { __: r, u: e } }),
			i = o[0].t,
			c = o[1];
		return (
			t.useLayoutEffect(
				function () {
					(i.__ = r), (i.u = e), u(i) && c({ t: i });
				},
				[n, r, e]
			),
			t.useEffect(
				function () {
					return (
						u(i) && c({ t: i }),
						n(function () {
							u(i) && c({ t: i });
						})
					);
				},
				[n]
			),
			r
		);
	}
	function u(n) {
		var e = n.u,
			t = n.__;
		try {
			var r = e();
			return !Object.is(t, r);
		} catch (n) {
			return !0;
		}
	}
	function o(n) {
		n();
	}
	function i(n) {
		return n;
	}
	function c() {
		return [!1, o];
	}
	var l = t.useLayoutEffect,
		f = Object.assign;
	function a(n, e) {
		for (var t in n) if ('__source' !== t && !(t in e)) return !0;
		for (var r in e) if ('__source' !== r && n[r] !== e[r]) return !0;
		return !1;
	}
	var s =
		/^(-|f[lo].*[^se]$|g.{5,}[^ps]$|z|o[pr]|(W.{5})?[lL]i.*(t|mp)$|an|(bo|s).{4}Im|sca|m.{6}[ds]|ta|c.*[st]$|wido|ini)/;
	function d(n, e) {
		(this.props = n), (this.context = e);
	}
	function p(n, t) {
		function r(n) {
			var e = this.props.ref,
				r = e == n.ref;
			return (
				!r && e && (e.call ? e(null) : (e.current = null)),
				t ? !t(this.props, n) || !r : a(this.props, n)
			);
		}
		function u(t) {
			return (this.shouldComponentUpdate = r), e.createElement(n, t);
		}
		return (
			(u.displayName = 'Memo(' + (n.displayName || n.name) + ')'),
			(u.prototype.isReactComponent = !0),
			(u.type = n),
			u
		);
	}
	((d.prototype = new e.Component()).isPureReactComponent = !0),
		(d.prototype.shouldComponentUpdate = function (n, e) {
			return a(this.props, n) || a(this.state, e);
		});
	var v = Symbol.for('react.forward_ref');
	function h(n) {
		function e(e) {
			var t = f({}, e);
			return delete t.ref, n(t, e.ref || null);
		}
		return (
			(e.$$typeof = v),
			(e.render = n),
			(e.prototype.isReactComponent = !0),
			(e.displayName = 'ForwardRef(' + (n.displayName || n.name) + ')'),
			e
		);
	}
	var m = function (n, t, r) {
			return null == n
				? null
				: e.toChildArray(e.toChildArray(n).map(t.bind(r)));
		},
		b = {
			map: m,
			forEach: m,
			count: function (n) {
				return n ? e.toChildArray(n).length : 0;
			},
			only: function (n) {
				var t = e.toChildArray(n);
				if (1 !== t.length) throw 'Children.only';
				return t[0];
			},
			toArray: e.toChildArray
		},
		_ = e.options.__e;
	e.options.__e = function (n, e, t, r) {
		if (n.then)
			for (var u, o = e; (o = o.__); )
				if ((u = o.__c) && u.__c)
					return (
						null == e.__e && ((e.__e = t.__e), (e.__k = t.__k)), u.__c(n, e)
					);
		_(n, e, t, r);
	};
	var y = e.options.unmount;
	function g(n, e, t) {
		return (
			n &&
				(n.__c &&
					n.__c.__H &&
					(n.__c.__H.__.forEach(function (n) {
						'function' == typeof n.__c && n.__c();
					}),
					(n.__c.__H = null)),
				null != (n = f({}, n)).__c &&
					(n.__c.__P === t && (n.__c.__P = e),
					(n.__c.__g |= 4),
					(n.__c = null)),
				(n.__k =
					n.__k &&
					n.__k.map(function (n) {
						return g(n, e, t);
					}))),
			n
		);
	}
	function E(n, e, t) {
		return (
			n &&
				t &&
				('string' == typeof n.type && (n.__u |= 1),
				(n.__v = null),
				(n.__k =
					n.__k &&
					n.__k.map(function (n) {
						return E(n, e, t);
					})),
				n.__c &&
					n.__c.__P === e &&
					(n.__e && t.appendChild(n.__e), (n.__c.__g |= 4), (n.__c.__P = t))),
			n
		);
	}
	function S() {
		(this.__u = 0), (this.o = null), (this.__b = null);
	}
	function C(n) {
		var t,
			r,
			u,
			o = null;
		function i(i) {
			if (
				(t ||
					(t = n()).then(
						function (n) {
							n && (o = n.default || n), (u = !0);
						},
						function (n) {
							(r = n), (u = !0);
						}
					),
				r)
			)
				throw r;
			if (!u) throw t;
			return o ? e.createElement(o, i) : null;
		}
		return (i.displayName = 'Lazy'), i;
	}
	function O(n) {
		return (
			(this.getChildContext = function () {
				return n.context;
			}),
			n.children
		);
	}
	function x(n) {
		var t = this,
			r = n.i;
		if (
			((t.componentWillUnmount = function () {
				e.render(null, t.l), (t.l = null), (t.i = null);
			}),
			t.i && t.i !== r && t.componentWillUnmount(),
			!t.l)
		) {
			for (var u = t.__v; null !== u && !u.__m && null !== u.__; ) u = u.__;
			(t.i = r),
				(t.l = {
					nodeType: 1,
					parentNode: r,
					childNodes: [],
					__k: { __m: u.__m },
					ownerDocument: r.ownerDocument,
					insertBefore: function (n, e) {
						this.childNodes.push(n), t.i.insertBefore(n, e);
					}
				});
		}
		e.render(e.createElement(O, { context: t.context }, n.__v), t.l);
	}
	function R(n, t) {
		var r = e.createElement(x, { __v: n, i: t });
		return (r.containerInfo = t), r;
	}
	(e.options.unmount = function (n) {
		var e = n.__c;
		e && e.__R && e.__R(), y && y(n);
	}),
		((S.prototype = new e.Component()).__c = function (n, e) {
			var t = e.__c,
				r = this;
			null == r.o && (r.o = []), r.o.push(t);
			var u = !1,
				o = function () {
					u || ((u = !0), (t.__R = null), i());
				};
			t.__R = o;
			var i = function () {
				if (!--r.__u) {
					if (r.state.__a) {
						var n = r.state.__a;
						r.__v.__k[0] = E(n, n.__c.__P, n.__c.__O);
					}
					var e;
					for (r.setState({ __a: (r.__b = null) }); (e = r.o.pop()); )
						e.forceUpdate();
				}
			};
			r.__u++ || 32 & e.__u || r.setState({ __a: (r.__b = r.__v.__k[0]) }),
				n.then(o, o);
		}),
		(S.prototype.componentWillUnmount = function () {
			this.o = [];
		}),
		(S.prototype.render = function (n, t) {
			if (this.__b) {
				if (this.__v.__k) {
					var r = document.createElement('div'),
						u = this.__v.__k[0].__c;
					this.__v.__k[0] = g(this.__b, r, (u.__O = u.__P));
				}
				this.__b = null;
			}
			return [
				e.createElement(e.Fragment, null, t.__a ? null : n.children),
				t.__a && e.createElement(e.Fragment, null, n.fallback)
			];
		});
	var j = Symbol.for('react.element'),
		w =
			/^(?:accent|alignment|arabic|baseline|cap|clip(?!PathU)|color|dominant|fill|flood|font|glyph(?!R)|horiz|image(!S)|letter|lighting|marker(?!H|W|U)|overline|paint|pointer|shape|stop|strikethrough|stroke|text(?!L)|transform|underline|unicode|units|v|vector|vert|word|writing|x(?!C))[A-Z]/,
		k = /[A-Z0-9]/g,
		I = 'undefined' != typeof document,
		N = function (n) {
			return /fil|che|rad/.test(n);
		};
	function D(n, t, r) {
		return (
			null == t.__k && (t.textContent = ''),
			e.render(n, t),
			'function' == typeof r && r(),
			n ? n.__c : null
		);
	}
	function T(n, t, r) {
		return e.hydrate(n, t), 'function' == typeof r && r(), n ? n.__c : null;
	}
	(e.Component.prototype.isReactComponent = {}),
		[
			'componentWillMount',
			'componentWillReceiveProps',
			'componentWillUpdate'
		].forEach(function (n) {
			Object.defineProperty(e.Component.prototype, n, {
				configurable: !0,
				get: function () {
					return this['UNSAFE_' + n];
				},
				set: function (e) {
					Object.defineProperty(this, n, {
						configurable: !0,
						writable: !0,
						value: e
					});
				}
			});
		});
	var L = e.options.event;
	function M() {}
	function A() {
		return this.cancelBubble;
	}
	function F() {
		return this.defaultPrevented;
	}
	e.options.event = function (n) {
		return (
			L && (n = L(n)),
			(n.persist = M),
			(n.isPropagationStopped = A),
			(n.isDefaultPrevented = F),
			(n.nativeEvent = n)
		);
	};
	var U,
		V = {
			enumerable: !1,
			configurable: !0,
			get: function () {
				return this.class;
			}
		},
		W = e.options.vnode;
	e.options.vnode = function (n) {
		if ('string' == typeof n.type)
			!(function (n) {
				var t = n.props,
					r = n.type,
					u = {},
					o = -1 === r.indexOf('-');
				for (var i in t) {
					var c = t[i];
					if (
						!(
							('value' === i && 'defaultValue' in t && null == c) ||
							(I && 'children' === i && 'noscript' === r) ||
							'class' === i ||
							'className' === i
						)
					) {
						if ('style' === i && 'object' == typeof c)
							for (var l in c)
								'number' != typeof c[l] || s.test(l) || (c[l] += 'px');
						else if ('defaultValue' === i && 'value' in t && null == t.value)
							i = 'value';
						else if ('download' === i && !0 === c) c = '';
						else if ('translate' === i && 'no' === c) c = !1;
						else if ('o' === i[0] && 'n' === i[1]) {
							var f = i.toLowerCase();
							'ondoubleclick' === f
								? (i = 'ondblclick')
								: 'onchange' !== f ||
										('input' !== r && 'textarea' !== r) ||
										N(t.type)
									? 'onfocus' === f
										? (i = 'onfocusin')
										: 'onblur' === f && (i = 'onfocusout')
									: (f = i = 'oninput'),
								'oninput' === f && u[(i = f)] && (i = 'oninputCapture');
						} else
							o && w.test(i)
								? (i = i.replace(k, '-$&').toLowerCase())
								: null === c && (c = void 0);
						u[i] = c;
					}
				}
				'select' == r &&
					u.multiple &&
					Array.isArray(u.value) &&
					(u.value = e.toChildArray(t.children).forEach(function (n) {
						n.props.selected = -1 != u.value.indexOf(n.props.value);
					})),
					'select' == r &&
						null != u.defaultValue &&
						(u.value = e.toChildArray(t.children).forEach(function (n) {
							n.props.selected = u.multiple
								? -1 != u.defaultValue.indexOf(n.props.value)
								: u.defaultValue == n.props.value;
						})),
					t.class && !t.className
						? ((u.class = t.class), Object.defineProperty(u, 'className', V))
						: ((t.className && !t.class) || (t.class && t.className)) &&
							(u.class = u.className = t.className),
					(n.props = u);
			})(n);
		else if (
			'function' == typeof n.type &&
			('ref' in n.props &&
				'prototype' in n.type &&
				n.type.prototype.render &&
				((n.ref = n.props.ref), delete n.props.ref),
			n.type.defaultProps)
		) {
			var t = f({}, n.props);
			for (var r in n.type.defaultProps)
				void 0 === t[r] && (t[r] = n.type.defaultProps[r]);
			n.props = t;
		}
		(n.$$typeof = j), W && W(n);
	};
	var $ = e.options.__r;
	e.options.__r = function (n) {
		$ && $(n), (U = n.__c);
	};
	var z = e.options.diffed;
	e.options.diffed = function (n) {
		z && z(n);
		var e = n.props,
			t = n.__e;
		null != t &&
			'textarea' === n.type &&
			'value' in e &&
			e.value !== t.value &&
			(t.value = null == e.value ? '' : e.value),
			(U = null);
	};
	var P = {
			ReactCurrentDispatcher: {
				current: {
					readContext: function (n) {
						return U.__n[n.__c].props.value;
					},
					useCallback: t.useCallback,
					useContext: t.useContext,
					useDebugValue: t.useDebugValue,
					useDeferredValue: i,
					useEffect: t.useEffect,
					useId: t.useId,
					useImperativeHandle: t.useImperativeHandle,
					useInsertionEffect: l,
					useLayoutEffect: t.useLayoutEffect,
					useMemo: t.useMemo,
					useReducer: t.useReducer,
					useRef: t.useRef,
					useState: t.useState,
					useSyncExternalStore: r,
					useTransition: c
				}
			}
		},
		H = '18.3.1';
	function q(n) {
		return e.createElement.bind(null, n);
	}
	function B(n) {
		return !!n && n.$$typeof === j;
	}
	function Z(n) {
		return B(n) && n.type === e.Fragment;
	}
	function Y(n) {
		return (
			!!n &&
			!!n.displayName &&
			('string' == typeof n.displayName || n.displayName instanceof String) &&
			n.displayName.startsWith('Memo(')
		);
	}
	function G(n) {
		return B(n) ? e.cloneElement.apply(null, arguments) : n;
	}
	function J(n) {
		return !!n.__k && (e.render(null, n), !0);
	}
	function K(n) {
		return (n && ((n.__v && n.__v.__e) || (1 === n.nodeType && n))) || null;
	}
	var Q = function (n, e) {
		return n(e);
	};
	function X(n, e) {
		return n(e);
	}
	var nn = e.Fragment,
		en = B,
		tn = {
			useState: t.useState,
			useId: t.useId,
			useReducer: t.useReducer,
			useEffect: t.useEffect,
			useLayoutEffect: t.useLayoutEffect,
			useInsertionEffect: l,
			useTransition: c,
			useDeferredValue: i,
			useSyncExternalStore: r,
			startTransition: o,
			useRef: t.useRef,
			useImperativeHandle: t.useImperativeHandle,
			useMemo: t.useMemo,
			useCallback: t.useCallback,
			useContext: t.useContext,
			useDebugValue: t.useDebugValue,
			version: H,
			Children: b,
			render: D,
			hydrate: T,
			unmountComponentAtNode: J,
			createPortal: R,
			createElement: e.createElement,
			createContext: e.createContext,
			createFactory: q,
			cloneElement: G,
			createRef: e.createRef,
			Fragment: e.Fragment,
			isValidElement: B,
			isElement: en,
			isFragment: Z,
			isMemo: Y,
			findDOMNode: K,
			Component: e.Component,
			PureComponent: d,
			memo: p,
			forwardRef: h,
			flushSync: Q,
			unstable_batchedUpdates: X,
			StrictMode: nn,
			Suspense: S,
			lazy: C,
			__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: P
		};
	Object.defineProperty(n, 'Component', {
		enumerable: !0,
		get: function () {
			return e.Component;
		}
	}),
		Object.defineProperty(n, 'Fragment', {
			enumerable: !0,
			get: function () {
				return e.Fragment;
			}
		}),
		Object.defineProperty(n, 'createContext', {
			enumerable: !0,
			get: function () {
				return e.createContext;
			}
		}),
		Object.defineProperty(n, 'createElement', {
			enumerable: !0,
			get: function () {
				return e.createElement;
			}
		}),
		Object.defineProperty(n, 'createRef', {
			enumerable: !0,
			get: function () {
				return e.createRef;
			}
		}),
		(n.Children = b),
		(n.PureComponent = d),
		(n.StrictMode = nn),
		(n.Suspense = S),
		(n.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED = P),
		(n.cloneElement = G),
		(n.createFactory = q),
		(n.createPortal = R),
		(n.default = tn),
		(n.findDOMNode = K),
		(n.flushSync = Q),
		(n.forwardRef = h),
		(n.hydrate = T),
		(n.isElement = en),
		(n.isFragment = Z),
		(n.isMemo = Y),
		(n.isValidElement = B),
		(n.lazy = C),
		(n.memo = p),
		(n.render = D),
		(n.startTransition = o),
		(n.unmountComponentAtNode = J),
		(n.unstable_batchedUpdates = X),
		(n.useDeferredValue = i),
		(n.useInsertionEffect = l),
		(n.useSyncExternalStore = r),
		(n.useTransition = c),
		(n.version = H),
		Object.keys(t).forEach(function (e) {
			'default' === e ||
				n.hasOwnProperty(e) ||
				Object.defineProperty(n, e, {
					enumerable: !0,
					get: function () {
						return t[e];
					}
				});
		});
});
//# sourceMappingURL=compat.umd.js.map
