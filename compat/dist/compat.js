var n = require('preact'),
	e = require('preact/hooks');
function t(n, t) {
	var u = t(),
		o = e.useState({ t: { __: u, u: t } }),
		i = o[0].t,
		c = o[1];
	return (
		e.useLayoutEffect(
			function () {
				(i.__ = u), (i.u = t), r(i) && c({ t: i });
			},
			[n, u, t]
		),
		e.useEffect(
			function () {
				return (
					r(i) && c({ t: i }),
					n(function () {
						r(i) && c({ t: i });
					})
				);
			},
			[n]
		),
		u
	);
}
function r(n) {
	var e = n.u,
		t = n.__;
	try {
		var r = e();
		return !Object.is(t, r);
	} catch (n) {
		return !0;
	}
}
function u(n) {
	n();
}
function o(n) {
	return n;
}
function i() {
	return [!1, u];
}
var c = e.useLayoutEffect,
	l = Object.assign;
function f(n, e) {
	for (var t in n) if ('__source' !== t && !(t in e)) return !0;
	for (var r in e) if ('__source' !== r && n[r] !== e[r]) return !0;
	return !1;
}
var a =
	/^(-|f[lo].*[^se]$|g.{5,}[^ps]$|z|o[pr]|(W.{5})?[lL]i.*(t|mp)$|an|(bo|s).{4}Im|sca|m.{6}[ds]|ta|c.*[st]$|wido|ini)/;
function s(n, e) {
	(this.props = n), (this.context = e);
}
function p(e, t) {
	function r(n) {
		var e = this.props.ref,
			r = e == n.ref;
		return (
			!r && e && (e.call ? e(null) : (e.current = null)),
			t ? !t(this.props, n) || !r : f(this.props, n)
		);
	}
	function u(t) {
		return (this.shouldComponentUpdate = r), n.createElement(e, t);
	}
	return (
		(u.displayName = 'Memo(' + (e.displayName || e.name) + ')'),
		(u.prototype.isReactComponent = !0),
		(u.type = e),
		u
	);
}
((s.prototype = new n.Component()).isPureReactComponent = !0),
	(s.prototype.shouldComponentUpdate = function (n, e) {
		return f(this.props, n) || f(this.state, e);
	});
var v = Symbol.for('react.forward_ref');
function d(n) {
	function e(e) {
		var t = l({}, e);
		return delete t.ref, n(t, e.ref || null);
	}
	return (
		(e.$$typeof = v),
		(e.render = n),
		(e.prototype.isReactComponent = !0),
		(e.displayName = 'ForwardRef(' + (n.displayName || n.name) + ')'),
		e
	);
}
var h = function (e, t, r) {
		return null == e ? null : n.toChildArray(n.toChildArray(e).map(t.bind(r)));
	},
	m = {
		map: h,
		forEach: h,
		count: function (e) {
			return e ? n.toChildArray(e).length : 0;
		},
		only: function (e) {
			var t = n.toChildArray(e);
			if (1 !== t.length) throw 'Children.only';
			return t[0];
		},
		toArray: n.toChildArray
	},
	x = n.options.__e;
n.options.__e = function (n, e, t, r) {
	if (n.then)
		for (var u, o = e; (o = o.__); )
			if ((u = o.__c) && u.__c)
				return null == e.__e && ((e.__e = t.__e), (e.__k = t.__k)), u.__c(n, e);
	x(n, e, t, r);
};
var b = n.options.unmount;
function _(n, e, t) {
	return (
		n &&
			(n.__c &&
				n.__c.__H &&
				(n.__c.__H.__.forEach(function (n) {
					'function' == typeof n.__c && n.__c();
				}),
				(n.__c.__H = null)),
			null != (n = l({}, n)).__c &&
				(n.__c.__P === t && (n.__c.__P = e), (n.__c.__g |= 4), (n.__c = null)),
			(n.__k =
				n.__k &&
				n.__k.map(function (n) {
					return _(n, e, t);
				}))),
		n
	);
}
function g(n, e, t) {
	return (
		n &&
			t &&
			('string' == typeof n.type && (n.__u |= 1),
			(n.__v = null),
			(n.__k =
				n.__k &&
				n.__k.map(function (n) {
					return g(n, e, t);
				})),
			n.__c &&
				n.__c.__P === e &&
				(n.__e && t.appendChild(n.__e), (n.__c.__g |= 4), (n.__c.__P = t))),
		n
	);
}
function y() {
	(this.__u = 0), (this.o = null), (this.__b = null);
}
function E(e) {
	var t,
		r,
		u,
		o = null;
	function i(i) {
		if (
			(t ||
				(t = e()).then(
					function (n) {
						n && (o = n.default || n), (u = !0);
					},
					function (n) {
						(r = n), (u = !0);
					}
				),
			r)
		)
			throw r;
		if (!u) throw t;
		return o ? n.createElement(o, i) : null;
	}
	return (i.displayName = 'Lazy'), i;
}
function S(n) {
	return (
		(this.getChildContext = function () {
			return n.context;
		}),
		n.children
	);
}
function C(e) {
	var t = this,
		r = e.i;
	if (
		((t.componentWillUnmount = function () {
			n.render(null, t.l), (t.l = null), (t.i = null);
		}),
		t.i && t.i !== r && t.componentWillUnmount(),
		!t.l)
	) {
		for (var u = t.__v; null !== u && !u.__m && null !== u.__; ) u = u.__;
		(t.i = r),
			(t.l = {
				nodeType: 1,
				parentNode: r,
				childNodes: [],
				__k: { __m: u.__m },
				ownerDocument: r.ownerDocument,
				insertBefore: function (n, e) {
					this.childNodes.push(n), t.i.insertBefore(n, e);
				}
			});
	}
	n.render(n.createElement(S, { context: t.context }, e.__v), t.l);
}
function O(e, t) {
	var r = n.createElement(C, { __v: e, i: t });
	return (r.containerInfo = t), r;
}
(n.options.unmount = function (n) {
	var e = n.__c;
	e && e.__R && e.__R(), b && b(n);
}),
	((y.prototype = new n.Component()).__c = function (n, e) {
		var t = e.__c,
			r = this;
		null == r.o && (r.o = []), r.o.push(t);
		var u = !1,
			o = function () {
				u || ((u = !0), (t.__R = null), i());
			};
		t.__R = o;
		var i = function () {
			if (!--r.__u) {
				if (r.state.__a) {
					var n = r.state.__a;
					r.__v.__k[0] = g(n, n.__c.__P, n.__c.__O);
				}
				var e;
				for (r.setState({ __a: (r.__b = null) }); (e = r.o.pop()); )
					e.forceUpdate();
			}
		};
		r.__u++ || 32 & e.__u || r.setState({ __a: (r.__b = r.__v.__k[0]) }),
			n.then(o, o);
	}),
	(y.prototype.componentWillUnmount = function () {
		this.o = [];
	}),
	(y.prototype.render = function (e, t) {
		if (this.__b) {
			if (this.__v.__k) {
				var r = document.createElement('div'),
					u = this.__v.__k[0].__c;
				this.__v.__k[0] = _(this.__b, r, (u.__O = u.__P));
			}
			this.__b = null;
		}
		return [
			n.createElement(n.Fragment, null, t.__a ? null : e.children),
			t.__a && n.createElement(n.Fragment, null, e.fallback)
		];
	});
var R = Symbol.for('react.element'),
	w =
		/^(?:accent|alignment|arabic|baseline|cap|clip(?!PathU)|color|dominant|fill|flood|font|glyph(?!R)|horiz|image(!S)|letter|lighting|marker(?!H|W|U)|overline|paint|pointer|shape|stop|strikethrough|stroke|text(?!L)|transform|underline|unicode|units|v|vector|vert|word|writing|x(?!C))[A-Z]/,
	j = /[A-Z0-9]/g,
	I = 'undefined' != typeof document,
	N = function (n) {
		return /fil|che|rad/.test(n);
	};
function k(e, t, r) {
	return (
		null == t.__k && (t.textContent = ''),
		n.render(e, t),
		'function' == typeof r && r(),
		e ? e.__c : null
	);
}
function D(e, t, r) {
	return n.hydrate(e, t), 'function' == typeof r && r(), e ? e.__c : null;
}
(n.Component.prototype.isReactComponent = {}),
	[
		'componentWillMount',
		'componentWillReceiveProps',
		'componentWillUpdate'
	].forEach(function (e) {
		Object.defineProperty(n.Component.prototype, e, {
			configurable: !0,
			get: function () {
				return this['UNSAFE_' + e];
			},
			set: function (n) {
				Object.defineProperty(this, e, {
					configurable: !0,
					writable: !0,
					value: n
				});
			}
		});
	});
var L = n.options.event;
function M() {}
function A() {
	return this.cancelBubble;
}
function F() {
	return this.defaultPrevented;
}
n.options.event = function (n) {
	return (
		L && (n = L(n)),
		(n.persist = M),
		(n.isPropagationStopped = A),
		(n.isDefaultPrevented = F),
		(n.nativeEvent = n)
	);
};
var T,
	U = {
		enumerable: !1,
		configurable: !0,
		get: function () {
			return this.class;
		}
	},
	V = n.options.vnode;
n.options.vnode = function (e) {
	if ('string' == typeof e.type)
		!(function (e) {
			var t = e.props,
				r = e.type,
				u = {},
				o = -1 === r.indexOf('-');
			for (var i in t) {
				var c = t[i];
				if (
					!(
						('value' === i && 'defaultValue' in t && null == c) ||
						(I && 'children' === i && 'noscript' === r) ||
						'class' === i ||
						'className' === i
					)
				) {
					if ('style' === i && 'object' == typeof c)
						for (var l in c)
							'number' != typeof c[l] || a.test(l) || (c[l] += 'px');
					else if ('defaultValue' === i && 'value' in t && null == t.value)
						i = 'value';
					else if ('download' === i && !0 === c) c = '';
					else if ('translate' === i && 'no' === c) c = !1;
					else if ('o' === i[0] && 'n' === i[1]) {
						var f = i.toLowerCase();
						'ondoubleclick' === f
							? (i = 'ondblclick')
							: 'onchange' !== f ||
									('input' !== r && 'textarea' !== r) ||
									N(t.type)
								? 'onfocus' === f
									? (i = 'onfocusin')
									: 'onblur' === f && (i = 'onfocusout')
								: (f = i = 'oninput'),
							'oninput' === f && u[(i = f)] && (i = 'oninputCapture');
					} else
						o && w.test(i)
							? (i = i.replace(j, '-$&').toLowerCase())
							: null === c && (c = void 0);
					u[i] = c;
				}
			}
			'select' == r &&
				u.multiple &&
				Array.isArray(u.value) &&
				(u.value = n.toChildArray(t.children).forEach(function (n) {
					n.props.selected = -1 != u.value.indexOf(n.props.value);
				})),
				'select' == r &&
					null != u.defaultValue &&
					(u.value = n.toChildArray(t.children).forEach(function (n) {
						n.props.selected = u.multiple
							? -1 != u.defaultValue.indexOf(n.props.value)
							: u.defaultValue == n.props.value;
					})),
				t.class && !t.className
					? ((u.class = t.class), Object.defineProperty(u, 'className', U))
					: ((t.className && !t.class) || (t.class && t.className)) &&
						(u.class = u.className = t.className),
				(e.props = u);
		})(e);
	else if (
		'function' == typeof e.type &&
		('ref' in e.props &&
			'prototype' in e.type &&
			e.type.prototype.render &&
			((e.ref = e.props.ref), delete e.props.ref),
		e.type.defaultProps)
	) {
		var t = l({}, e.props);
		for (var r in e.type.defaultProps)
			void 0 === t[r] && (t[r] = e.type.defaultProps[r]);
		e.props = t;
	}
	(e.$$typeof = R), V && V(e);
};
var W = n.options.__r;
n.options.__r = function (n) {
	W && W(n), (T = n.__c);
};
var $ = n.options.diffed;
n.options.diffed = function (n) {
	$ && $(n);
	var e = n.props,
		t = n.__e;
	null != t &&
		'textarea' === n.type &&
		'value' in e &&
		e.value !== t.value &&
		(t.value = null == e.value ? '' : e.value),
		(T = null);
};
var z = {
	ReactCurrentDispatcher: {
		current: {
			readContext: function (n) {
				return T.__n[n.__c].props.value;
			},
			useCallback: e.useCallback,
			useContext: e.useContext,
			useDebugValue: e.useDebugValue,
			useDeferredValue: o,
			useEffect: e.useEffect,
			useId: e.useId,
			useImperativeHandle: e.useImperativeHandle,
			useInsertionEffect: c,
			useLayoutEffect: e.useLayoutEffect,
			useMemo: e.useMemo,
			useReducer: e.useReducer,
			useRef: e.useRef,
			useState: e.useState,
			useSyncExternalStore: t,
			useTransition: i
		}
	}
};
function P(e) {
	return n.createElement.bind(null, e);
}
function H(n) {
	return !!n && n.$$typeof === R;
}
function q(e) {
	return H(e) && e.type === n.Fragment;
}
function B(n) {
	return (
		!!n &&
		!!n.displayName &&
		('string' == typeof n.displayName || n.displayName instanceof String) &&
		n.displayName.startsWith('Memo(')
	);
}
function Z(e) {
	return H(e) ? n.cloneElement.apply(null, arguments) : e;
}
function Y(e) {
	return !!e.__k && (n.render(null, e), !0);
}
function G(n) {
	return (n && ((n.__v && n.__v.__e) || (1 === n.nodeType && n))) || null;
}
var J = function (n, e) {
	return n(e);
};
function K(n, e) {
	return n(e);
}
var Q = n.Fragment,
	X = H,
	nn = {
		useState: e.useState,
		useId: e.useId,
		useReducer: e.useReducer,
		useEffect: e.useEffect,
		useLayoutEffect: e.useLayoutEffect,
		useInsertionEffect: c,
		useTransition: i,
		useDeferredValue: o,
		useSyncExternalStore: t,
		startTransition: u,
		useRef: e.useRef,
		useImperativeHandle: e.useImperativeHandle,
		useMemo: e.useMemo,
		useCallback: e.useCallback,
		useContext: e.useContext,
		useDebugValue: e.useDebugValue,
		version: '18.3.1',
		Children: m,
		render: k,
		hydrate: D,
		unmountComponentAtNode: Y,
		createPortal: O,
		createElement: n.createElement,
		createContext: n.createContext,
		createFactory: P,
		cloneElement: Z,
		createRef: n.createRef,
		Fragment: n.Fragment,
		isValidElement: H,
		isElement: X,
		isFragment: q,
		isMemo: B,
		findDOMNode: G,
		Component: n.Component,
		PureComponent: s,
		memo: p,
		forwardRef: d,
		flushSync: J,
		unstable_batchedUpdates: K,
		StrictMode: Q,
		Suspense: y,
		lazy: E,
		__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: z
	};
Object.defineProperty(exports, 'Component', {
	enumerable: !0,
	get: function () {
		return n.Component;
	}
}),
	Object.defineProperty(exports, 'Fragment', {
		enumerable: !0,
		get: function () {
			return n.Fragment;
		}
	}),
	Object.defineProperty(exports, 'createContext', {
		enumerable: !0,
		get: function () {
			return n.createContext;
		}
	}),
	Object.defineProperty(exports, 'createElement', {
		enumerable: !0,
		get: function () {
			return n.createElement;
		}
	}),
	Object.defineProperty(exports, 'createRef', {
		enumerable: !0,
		get: function () {
			return n.createRef;
		}
	}),
	(exports.Children = m),
	(exports.PureComponent = s),
	(exports.StrictMode = Q),
	(exports.Suspense = y),
	(exports.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED = z),
	(exports.cloneElement = Z),
	(exports.createFactory = P),
	(exports.createPortal = O),
	(exports.default = nn),
	(exports.findDOMNode = G),
	(exports.flushSync = J),
	(exports.forwardRef = d),
	(exports.hydrate = D),
	(exports.isElement = X),
	(exports.isFragment = q),
	(exports.isMemo = B),
	(exports.isValidElement = H),
	(exports.lazy = E),
	(exports.memo = p),
	(exports.render = k),
	(exports.startTransition = u),
	(exports.unmountComponentAtNode = Y),
	(exports.unstable_batchedUpdates = K),
	(exports.useDeferredValue = o),
	(exports.useInsertionEffect = c),
	(exports.useSyncExternalStore = t),
	(exports.useTransition = i),
	(exports.version = '18.3.1'),
	Object.keys(e).forEach(function (n) {
		'default' === n ||
			exports.hasOwnProperty(n) ||
			Object.defineProperty(exports, n, {
				enumerable: !0,
				get: function () {
					return e[n];
				}
			});
	});
//# sourceMappingURL=compat.js.map
