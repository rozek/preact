!(function (n, t) {
	'object' == typeof exports && 'undefined' != typeof module
		? t(exports, require('preact'))
		: 'function' == typeof define && define.amd
			? define(['exports', 'preact'], t)
			: t(((n || self).preactHooks = {}), n.preact);
})(this, function (n, t) {
	var u,
		i,
		r,
		o,
		f = Object.is,
		c = 0,
		e = [],
		a = t.options,
		v = a.__b,
		l = a.__r,
		d = a.diffed,
		p = a.__c,
		s = a.unmount,
		y = a.__;
	function h(n, t) {
		a.__h && a.__h(i, n, c || t), (c = 0);
		var u = i.__H || (i.__H = { __: [], __h: [] });
		return n >= u.__.length && u.__.push({}), u.__[n];
	}
	function m(n) {
		return (c = 1), T(O, n);
	}
	function T(n, t, r) {
		var o = h(u++, 2);
		if (
			((o.t = n),
			!o.__c &&
				((o.__ = [
					r ? r(t) : O(void 0, t),
					function (n) {
						var t = o.__N ? o.__N[0] : o.__[0],
							u = o.t(t, n);
						f(t, u) || ((o.__N = [u, o.__[1]]), o.__c.setState({}));
					}
				]),
				(o.__c = i),
				!i.__f))
		) {
			var c = function (n, t, u) {
				if (!o.__c.__H) return !0;
				var i = o.__c.__H.__,
					r =
						o.__c.props !== n ||
						i.every(function (n) {
							return !n.__N;
						});
				return (
					i.forEach(function (n) {
						if (n.__N) {
							var t = n.__[0];
							(n.__ = n.__N), (n.__N = void 0), f(t, n.__[0]) || (r = !0);
						}
					}),
					(e && e.call(this, n, t, u)) || r
				);
			};
			i.__f = !0;
			var e = i.shouldComponentUpdate,
				a = i.componentWillUpdate;
			(i.componentWillUpdate = function (n, t, u) {
				if (4 & this.__g) {
					var i = e;
					(e = void 0), c(n, t, u), (e = i);
				}
				a && a.call(this, n, t, u);
			}),
				(i.shouldComponentUpdate = c);
		}
		return o.__;
	}
	function b(n, t) {
		var r = h(u++, 4);
		!a.__s && j(r.__H, t) && ((r.__ = n), (r.u = t), i.__h.push(r));
	}
	function _(n, t) {
		var i = h(u++, 7);
		return j(i.__H, t) && ((i.__ = n()), (i.__H = t), (i.__h = n)), i.__;
	}
	function q() {
		for (var n; (n = e.shift()); )
			if (n.__P && n.__H)
				try {
					n.__H.__h.forEach(F), n.__H.__h.forEach(g), (n.__H.__h = []);
				} catch (t) {
					(n.__H.__h = []), a.__e(t, n.__v);
				}
	}
	(a.__b = function (n) {
		(i = null), v && v(n);
	}),
		(a.__ = function (n, t) {
			n && t.__k && t.__k.__m && (n.__m = t.__k.__m), y && y(n, t);
		}),
		(a.__r = function (n) {
			l && l(n), (u = 0);
			var t = (i = n.__c).__H;
			t &&
				(r === i
					? ((t.__h = []),
						(i.__h = []),
						t.__.forEach(function (n) {
							n.__N && (n.__ = n.__N), (n.u = n.__N = void 0);
						}))
					: (t.__h.forEach(F), t.__h.forEach(g), (t.__h = []), (u = 0))),
				(r = i);
		}),
		(a.diffed = function (n) {
			d && d(n);
			var t = n.__c;
			t &&
				t.__H &&
				(t.__H.__h.length &&
					((1 !== e.push(t) && o === a.requestAnimationFrame) ||
						((o = a.requestAnimationFrame) || A)(q)),
				t.__H.__.forEach(function (n) {
					n.u && (n.__H = n.u), (n.u = void 0);
				})),
				(r = i = null);
		}),
		(a.__c = function (n, t) {
			t.some(function (n) {
				try {
					n.__h.forEach(F),
						(n.__h = n.__h.filter(function (n) {
							return !n.__ || g(n);
						}));
				} catch (u) {
					t.some(function (n) {
						n.__h && (n.__h = []);
					}),
						(t = []),
						a.__e(u, n.__v);
				}
			}),
				p && p(n, t);
		}),
		(a.unmount = function (n) {
			s && s(n);
			var t,
				u = n.__c;
			u &&
				u.__H &&
				(u.__H.__.forEach(function (n) {
					try {
						F(n);
					} catch (n) {
						t = n;
					}
				}),
				(u.__H = void 0),
				t && a.__e(t, u.__v));
		});
	var x = 'function' == typeof requestAnimationFrame;
	function A(n) {
		var t,
			u = function () {
				clearTimeout(i), x && cancelAnimationFrame(t), setTimeout(n);
			},
			i = setTimeout(u, 35);
		x && (t = requestAnimationFrame(u));
	}
	function F(n) {
		var t = i,
			u = n.__c;
		'function' == typeof u && ((n.__c = void 0), u()), (i = t);
	}
	function g(n) {
		var t = i;
		(n.__c = n.__()), (i = t);
	}
	function j(n, t) {
		return (
			!n ||
			n.length !== t.length ||
			t.some(function (t, u) {
				return !f(t, n[u]);
			})
		);
	}
	function O(n, t) {
		return 'function' == typeof t ? t(n) : t;
	}
	(n.useCallback = function (n, t) {
		return (
			(c = 8),
			_(function () {
				return n;
			}, t)
		);
	}),
		(n.useContext = function (n) {
			var t = i.context[n.__c],
				r = h(u++, 9);
			return (
				(r.c = n),
				t ? (null == r.__ && ((r.__ = !0), t.sub(i)), t.props.value) : n.__
			);
		}),
		(n.useDebugValue = function (n, t) {
			a.useDebugValue && a.useDebugValue(t ? t(n) : n);
		}),
		(n.useEffect = function (n, t) {
			var r = h(u++, 3);
			!a.__s && j(r.__H, t) && ((r.__ = n), (r.u = t), i.__H.__h.push(r));
		}),
		(n.useErrorBoundary = function (n) {
			var t = h(u++, 10),
				r = m();
			return (
				(t.__ = n),
				i.componentDidCatch ||
					(i.componentDidCatch = function (n, u) {
						t.__ && t.__(n, u), r[1](n);
					}),
				[
					r[0],
					function () {
						r[1](void 0);
					}
				]
			);
		}),
		(n.useId = function () {
			var n = h(u++, 11);
			if (!n.__) {
				for (var t = i.__v; null !== t && !t.__m && null !== t.__; ) t = t.__;
				var r = t.__m || (t.__m = [0, 0]);
				n.__ = 'P' + r[0] + '-' + r[1]++;
			}
			return n.__;
		}),
		(n.useImperativeHandle = function (n, t, u) {
			(c = 6),
				b(
					function () {
						if ('function' == typeof n) {
							var u = n(t());
							return function () {
								n(null), u && 'function' == typeof u && u();
							};
						}
						if (n)
							return (
								(n.current = t()),
								function () {
									return (n.current = null);
								}
							);
					},
					null == u ? u : u.concat(n)
				);
		}),
		(n.useLayoutEffect = b),
		(n.useMemo = _),
		(n.useReducer = T),
		(n.useRef = function (n) {
			return (
				(c = 5),
				_(function () {
					return { current: n };
				}, [])
			);
		}),
		(n.useState = m);
});
//# sourceMappingURL=hooks.umd.js.map
