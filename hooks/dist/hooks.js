var n,
	t,
	r,
	u,
	o = require('preact'),
	i = Object.is,
	f = 0,
	c = [],
	e = o.options,
	a = e.__b,
	v = e.__r,
	s = e.diffed,
	l = e.__c,
	p = e.unmount,
	x = e.__;
function m(n, r) {
	e.__h && e.__h(t, n, f || r), (f = 0);
	var u = t.__H || (t.__H = { __: [], __h: [] });
	return n >= u.__.length && u.__.push({}), u.__[n];
}
function d(n) {
	return (f = 1), y(O, n);
}
function y(r, u, o) {
	var f = m(n++, 2);
	if (
		((f.t = r),
		!f.__c &&
			((f.__ = [
				o ? o(u) : O(void 0, u),
				function (n) {
					var t = f.__N ? f.__N[0] : f.__[0],
						r = f.t(t, n);
					i(t, r) || ((f.__N = [r, f.__[1]]), f.__c.setState({}));
				}
			]),
			(f.__c = t),
			!t.__f))
	) {
		var c = function (n, t, r) {
			if (!f.__c.__H) return !0;
			var u = f.__c.__H.__,
				o =
					f.__c.props !== n ||
					u.every(function (n) {
						return !n.__N;
					});
			return (
				u.forEach(function (n) {
					if (n.__N) {
						var t = n.__[0];
						(n.__ = n.__N), (n.__N = void 0), i(t, n.__[0]) || (o = !0);
					}
				}),
				(e && e.call(this, n, t, r)) || o
			);
		};
		t.__f = !0;
		var e = t.shouldComponentUpdate,
			a = t.componentWillUpdate;
		(t.componentWillUpdate = function (n, t, r) {
			if (4 & this.__g) {
				var u = e;
				(e = void 0), c(n, t, r), (e = u);
			}
			a && a.call(this, n, t, r);
		}),
			(t.shouldComponentUpdate = c);
	}
	return f.__;
}
function h(r, u) {
	var o = m(n++, 4);
	!e.__s && j(o.__H, u) && ((o.__ = r), (o.u = u), t.__h.push(o));
}
function _(t, r) {
	var u = m(n++, 7);
	return j(u.__H, r) && ((u.__ = t()), (u.__H = r), (u.__h = t)), u.__;
}
function q() {
	for (var n; (n = c.shift()); )
		if (n.__P && n.__H)
			try {
				n.__H.__h.forEach(T), n.__H.__h.forEach(b), (n.__H.__h = []);
			} catch (t) {
				(n.__H.__h = []), e.__e(t, n.__v);
			}
}
(e.__b = function (n) {
	(t = null), a && a(n);
}),
	(e.__ = function (n, t) {
		n && t.__k && t.__k.__m && (n.__m = t.__k.__m), x && x(n, t);
	}),
	(e.__r = function (u) {
		v && v(u), (n = 0);
		var o = (t = u.__c).__H;
		o &&
			(r === t
				? ((o.__h = []),
					(t.__h = []),
					o.__.forEach(function (n) {
						n.__N && (n.__ = n.__N), (n.u = n.__N = void 0);
					}))
				: (o.__h.forEach(T), o.__h.forEach(b), (o.__h = []), (n = 0))),
			(r = t);
	}),
	(e.diffed = function (n) {
		s && s(n);
		var o = n.__c;
		o &&
			o.__H &&
			(o.__H.__h.length &&
				((1 !== c.push(o) && u === e.requestAnimationFrame) ||
					((u = e.requestAnimationFrame) || F)(q)),
			o.__H.__.forEach(function (n) {
				n.u && (n.__H = n.u), (n.u = void 0);
			})),
			(r = t = null);
	}),
	(e.__c = function (n, t) {
		t.some(function (n) {
			try {
				n.__h.forEach(T),
					(n.__h = n.__h.filter(function (n) {
						return !n.__ || b(n);
					}));
			} catch (r) {
				t.some(function (n) {
					n.__h && (n.__h = []);
				}),
					(t = []),
					e.__e(r, n.__v);
			}
		}),
			l && l(n, t);
	}),
	(e.unmount = function (n) {
		p && p(n);
		var t,
			r = n.__c;
		r &&
			r.__H &&
			(r.__H.__.forEach(function (n) {
				try {
					T(n);
				} catch (n) {
					t = n;
				}
			}),
			(r.__H = void 0),
			t && e.__e(t, r.__v));
	});
var A = 'function' == typeof requestAnimationFrame;
function F(n) {
	var t,
		r = function () {
			clearTimeout(u), A && cancelAnimationFrame(t), setTimeout(n);
		},
		u = setTimeout(r, 35);
	A && (t = requestAnimationFrame(r));
}
function T(n) {
	var r = t,
		u = n.__c;
	'function' == typeof u && ((n.__c = void 0), u()), (t = r);
}
function b(n) {
	var r = t;
	(n.__c = n.__()), (t = r);
}
function j(n, t) {
	return (
		!n ||
		n.length !== t.length ||
		t.some(function (t, r) {
			return !i(t, n[r]);
		})
	);
}
function O(n, t) {
	return 'function' == typeof t ? t(n) : t;
}
(exports.useCallback = function (n, t) {
	return (
		(f = 8),
		_(function () {
			return n;
		}, t)
	);
}),
	(exports.useContext = function (r) {
		var u = t.context[r.__c],
			o = m(n++, 9);
		return (
			(o.c = r),
			u ? (null == o.__ && ((o.__ = !0), u.sub(t)), u.props.value) : r.__
		);
	}),
	(exports.useDebugValue = function (n, t) {
		e.useDebugValue && e.useDebugValue(t ? t(n) : n);
	}),
	(exports.useEffect = function (r, u) {
		var o = m(n++, 3);
		!e.__s && j(o.__H, u) && ((o.__ = r), (o.u = u), t.__H.__h.push(o));
	}),
	(exports.useErrorBoundary = function (r) {
		var u = m(n++, 10),
			o = d();
		return (
			(u.__ = r),
			t.componentDidCatch ||
				(t.componentDidCatch = function (n, t) {
					u.__ && u.__(n, t), o[1](n);
				}),
			[
				o[0],
				function () {
					o[1](void 0);
				}
			]
		);
	}),
	(exports.useId = function () {
		var r = m(n++, 11);
		if (!r.__) {
			for (var u = t.__v; null !== u && !u.__m && null !== u.__; ) u = u.__;
			var o = u.__m || (u.__m = [0, 0]);
			r.__ = 'P' + o[0] + '-' + o[1]++;
		}
		return r.__;
	}),
	(exports.useImperativeHandle = function (n, t, r) {
		(f = 6),
			h(
				function () {
					if ('function' == typeof n) {
						var r = n(t());
						return function () {
							n(null), r && 'function' == typeof r && r();
						};
					}
					if (n)
						return (
							(n.current = t()),
							function () {
								return (n.current = null);
							}
						);
				},
				null == r ? r : r.concat(n)
			);
	}),
	(exports.useLayoutEffect = h),
	(exports.useMemo = _),
	(exports.useReducer = y),
	(exports.useRef = function (n) {
		return (
			(f = 5),
			_(function () {
				return { current: n };
			}, [])
		);
	}),
	(exports.useState = d);
//# sourceMappingURL=hooks.js.map
